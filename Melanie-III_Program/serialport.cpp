...
BOOL CSerialPort::Read(void* lpBuf, DWORD dwCount)
{
  ASSERT(IsOpen());
  ...
  if (!ReadFile(m_hComm, lpBuf, dwCount, &dwBytesRead, NULL))
  {
	...
	if (dwBytesRead<dwCount) bSuccess=FALSE;

  return bSuccess;
}

//DEL BOOL CSerialPort::Read(void* lpBuf, DWORD dwCount, OVERLAPPED& overlapped)
//DEL {
//DEL   ASSERT(IsOpen());
//DEL   ASSERT(m_bOverlapped);
//DEL   ASSERT(overlapped.hEvent);
//DEL 
//DEL   DWORD dwBytesRead = 0;
//DEL   BOOL bSuccess = ReadFile(m_hComm, lpBuf, dwCount, &dwBytesRead, &overlapped);
//DEL   if (!bSuccess)
//DEL   {
//DEL     if (GetLastError() != ERROR_IO_PENDING)
//DEL     {
//DEL       TRACE(_T("Failed in call to ReadFile\n"));
//DEL       AfxThrowSerialException();
//DEL     }
//DEL   }
//DEL   return bSuccess;
//DEL }
